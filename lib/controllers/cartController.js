...
exports.addToCart = async (req, res) => {
  ...
  try {
    ...
    if (itemIndex > -1) {
      ...
      if (normalizedSize) {
        cart.items[itemIndex].selectedSize = normalizedSize;
      }
    } else {
      cart.items.push({ product: productId, quantity, selectedSize: normalizedSize });
    }
...
exports.updateQuantity = async (req, res) => {
  ...
  try {
    ...
    if (selectedSize !== undefined) {
      item.selectedSize = String(selectedSize || '').trim();
    }
